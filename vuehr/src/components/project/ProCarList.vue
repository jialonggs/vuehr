  ...
  <div v-show="nowTab === 1">
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item><i class="el-icon-menu"></i>车型管理</el-breadcrumb-item>
        <el-breadcrumb-item>车型列表</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <el-container>
      <el-header style="padding: 0px;display:flex;justify-content:space-between;align-items: center;margin-top:20px;">
        <div class="handle-box">
          <el-button type="primary" icon="plus" class="mr10" @click="toadd()">添加车型</el-button>
      ...
      <el-main style="padding-left: 0px;padding-top: 0px">
        <el-table :data="data" stripe style="width: 100%" v-loading="tableLoading">
          ...
          <el-table-column  label="所属主机厂">
            <template slot-scope="scope">
              <span>{{scope.row.engine.name}}</span>
            </template>
          </el-table-column>
    ...
    <div class="form-box">
      <el-form ref="enginefrom" :model="enginefrom" :rules="rules" label-width="120px">
        <el-form-item label="车型名称:" prop="name">
          <el-input placeholder="请输入车型名称" v-model="enginefrom.name"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    updateEngine(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.postRequest("/car/update", self.enginefrom).then(resp => {
            this.dialogFormVisible = false;
            if (resp && resp.status == 200 && resp.data.code == 0) {
    ...
    addEngine(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.postRequest("/car/addCar", self.enginefrom).then(resp => {
    ...
    getCollectMouldList() {
      let _this = this
      this.getRequest("/car/listbypage?page=" + this.currentPage + "&size=" + this.pagesize).then(resp => {
        _this.tableLoading = false;
        if (resp && resp.status == 200 && resp.data.code == 0) {
          _this.tableData = resp.data.data.carlist

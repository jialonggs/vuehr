    ...
    <el-container>
      <el-main style="padding-left: 0px;padding-top: 20px">
        <div v-loading="base_loading">
          <div class="handle-box" v-if="isShangWu">
            <el-button type="primary"  @click="changeUrgency1()">修改订单进度</el-button>
          </div>
        ...
        <div v-loading="base_loading">
          <el-card style="margin-top:20px;">
           ...
           <!-- <el-button
            ...
            <div>
              <el-dialog width="60%" title="纹理详情" :visible.sync="innerVisible1" :close-on-click-modal="false" append-to-body>
            ...
            <div class="card-bottom">
              <el-button type="primary" @click="toUpdateBaseInfo('ruleForm')">修 改</el-button>
            </div>
          </el-card>
        </div>
        <!-- 选择模具 -->
          ...
          <el-card class="box-card" style="margin-top:25px;">
            <div slot="header" class="clearfix">
              ...
              <!-- <div style="float:right">
                    <el-button  type="primary" @click="toChangeSet">修 改</el-button>
                  </div> -->
            </div>
            <div>
              <el-row :gutter="30">
            ...
            <!-- <div class="card-bottom" style="float: left;">
              <el-button type="primary" @click="nowTab = 1"></el-button>
            </div> -->
            <div class="card-bottom">
              <el-button type="primary" @click="toUpdateSetMouldPart">修 改</el-button>
            </div>
          </el-card>
        </div>
        <!-- 下控制单 -->
        ...
        <div class="card-bottom">
          <el-button type="primary" @click="toUpBack">返回列表</el-button>
        </div>
      </el-main>
    </el-container>
    <!-- 添加表单 -->
    <el-dialog title="车型信息" :show-close="false" :visible.sync="urgencyDisable" width='40%'>
      <div class="form-box">
        <el-form ref="enginefrom" :model="changeUrgency" :rules="rules" label-width="120px">
          <el-form-item label="紧急程度：" prop="urgency">
            <el-select v-model='changeUrgency.urgency' placeholder="请选择紧急程度">
              <el-option label="特急" value='8'></el-option>
              <el-option label="紧急A" value='7'></el-option>
              <el-option label="紧急B" value='6'></el-option>
              <el-option label="紧急C" value='5'></el-option>
        ...
        <div slot="footer" class="dialog-footer">
            <el-button  type="primary"  @click="toChangeUrgency">确 定</el-button>
            <el-button @click="urgencyDisable = false">取 消</el-button>
        </div>
    </el-dialog>
  </div>
</div>
...
export default {
  ...
  methods: {
    ...
    toChangeUrgency(){
      let data = {
        orderId: this.urlId,
        urgency:this.changeUrgency.urgency
      }
      this.postRequest("/order/list/change/urgency", data).then(resp => {
        if (resp && resp.status == 200 && resp.data.code == 0) {
          this.$message.success("修改成功");
          this.urgencyDisable = false;
    ...
    toUpdateSetMouldPart() {
      ...
      this.postRequest("/order/list/set/mould", data).then(resp => {
        if (resp && resp.status == 200 && resp.data.code == 0) {
          self.$message.info("模具修改成功");
          self.getMouldTree();
        } else {
          self.$message.error("模具修改失败");
        }
      })
    },
    delMould(id){
      ...
      this.postRequest("/order/list/update/set/mould", params).then(resp => {
        if (resp && resp.status == 200 && resp.data.code == 0) {
          this.$message.success('模具信息更新成功');
          let item = {
            id:id
          }
          this.delSelected(item);
          this.getMouldTree();
        } else {
          this.$message.error('模具信息更新失败');
        }
      })
    },
    toDeleted(id) {
      this.$confirm('此操作将取消此模具的选择, 是否继续?', '提示', {
    ...
    toUpdateBaseInfo(formName) {
      let self = this;
      let url = "";
      this.$refs[formName].validate((valid) => {
        if (valid) {
          let paramsData = self.ruleForm;
          for (var i = 0; i < this.restaurants4.length; i++) {
            if(this.restaurants4[i].value === paramsData.managerId){
              paramsData.managerName = this.restaurants4[i].label
            }
          }
          ...
          this.jsonPostRequest("/order/list/update/order", paramsData).then(resp => {
            if (resp && resp.status == 200 && resp.data.code == 0) {
              self.$message.success('基础信息更新成功');
            } else {
              self.$message.error('基础信息更新失败');
            }
          })
        } else {
    ...
    toUpdate(){
      let self = this
      this.postRequest("/order/list/update/control", self.exform).then(resp => {
    ...
    delSelected(item) {
      ...
      let newS = [];
      for (let i = 0; i < sel.length; i++) {
        if (sel[i].collectPartinfoEntity.id+'' === item.id +'') {
          continue;
        } else {
          newS.push(sel[i]);
        }
    ...
    todelImage(url) {
      ...
      this.getRequest("/config/delfile/delImage?url=" + url).then(resp => {
        if (resp && resp.status == 200 && resp.data.code == 0) {
          ...
          this.toUpdateBaseInfo('ruleForm')
        } else {
      ...
      this.getRequest("/order/list/info?orderId=" + urlId).then(resp => {
        if (resp && resp.status == 200 && resp.data.code == 0) {
          ...
          && resp.data.data.orderinfo.picUrls.length>0){
            ...
            let urls = te(resp.data.data.orderinfo.picUrls)
            this.imageUrls = urls
          }
          ...
          this.wenliData = this.ruleForm.wenlis
          if('undefined' != moulds && moulds.length > 0){
  ...
  computed: {
    isShangWu: function () {
      ...
      var isShangWu = false;
      roles.forEach(role=> {
        if (role.name == 'ROLE_admin' || role.name == 'ROLE_shangwu') {
          isShangWu = true;
        }
      })
      return isShangWu;

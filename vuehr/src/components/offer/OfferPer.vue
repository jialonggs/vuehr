  ...
  <div v-show="showTab ==1">
    <div class="crumbs" >
      <el-breadcrumb separator="/">
        <el-breadcrumb-item><i class="el-icon-menu"></i>预报价</el-breadcrumb-item>
        <el-breadcrumb-item>预报价列表</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <el-container>
      <el-header style="padding: 0px;display:flex;justify-content:space-between;align-items: center;margin-top:30px;">
        <div class="handle-box">
          <el-button type="primary" icon="plus" class="mr10" @click="toadd">增添预报价单</el-button>
      ...
      <el-main style="padding-left: 0px;padding-top: 0px;margin-top:20px;">
        <div>
          <el-table :data="tableData" stripe  v-loading="tableLoading" style="width: 100%">
            ...
            <!-- <el-table-column label="含税合计" prop="total">
            </el-table-column> -->
          ...
          <el-button
            ...
            <!-- <el-button
              size="mini"
              type="warning"
              @click="mojuInfo(scope.row)">产品</el-button> -->
        </template>
            </el-table-column>
          </el-table>
        </div>
...
export default {
  ...
  methods: {
    ...
    toAddYouHui(){
      ...
      this.postRequest("/offer/per/update/final/baojia", params).then(resp => {
    ...
    toinfo(id) {
      this.$router.push('/offer/per/info/' + id)
    },
    toadd() {
    ...
    getCollectMouldList() {
      let _this = this
      this.getRequest("/offer/per/listbypage?page=" + this.currentPage + "&size=" + this.pagesize +"&addUserId=" +this.uid).then(resp => {

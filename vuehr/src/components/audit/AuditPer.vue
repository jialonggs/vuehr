...
export default {
  ...
  methods: {
    toinfo(id) {
      this.$router.push('/per/offer/info/' + id)
    },
    tongGuoShenHe(item){
    ...
    getCollectMouldList(audit, currentPage, pagesize) {
      let _this = this
      this.getRequest("/audit/per/listbypage?page=" + currentPage + "&size=" + pagesize + "&audit=" + audit).then(resp => {
        if (resp && resp.status == 200 && resp.data.code == 0) {
          if (audit == 0){
            _this.tableData0 = resp.data.data.audityubaojialist
            _this.totalnum0 = resp.data.data.count
          }else if(audit == 1){
            _this.tableData1 = resp.data.data.audityubaojialist
            _this.totalnum1 = resp.data.data.count
          }else if(audit == 2){
            _this.tableData2 = resp.data.data.audityubaojialist
            _this.totalnum1 = resp.data.data.count
          }
        }
      })

  ...
  <div>
    <div class="crumbs" >
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>待备料</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <el-container>
        ...
        <el-tag v-if="scope.row.status+'' === '2'" type="warning">完成</el-tag>
            ...
            <el-table-column prop="createTime" label="下单时间">
              ...
              <el-button
                size="mini"
                type="danger"
                @click="wancheng(scope.row)">备料完成</el-button>
            </template>
              </el-table-column>
          </el-table>
        </div>
  ...
  methods: {
    ...
    wancheng(item) {
      ...
      let params = {
        wuLiaoId:item.id,
        status:1,
        principalUserId: this.uid,
        principalUserName: this.name
      }
    ...
    getCollectMouldList() {
      let _this = this
      this.getRequest("/stock/manager/listbypage?page=" + this.currentPage + "&size=" + this.pagesize +"&status=0").then(resp => {

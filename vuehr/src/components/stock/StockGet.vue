...
<div>
  <div>
    <div class="crumbs" >
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>待领取</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <el-container>
        ...
        <el-tag v-if="scope.row.status+'' === '2'" type="warning">完成</el-tag>
            ...
            <el-table-column prop="createTime" label="下单时间">
              ...
              <el-button
                size="mini"
                type="danger"
                @click="toLingqu(scope.row)">领取</el-button>
            </template>
              </el-table-column>
          </el-table>
        </div>
  ...
  <el-dialog title="员工身份认证" v-loading="checkLoading"  :show-close="false" :close-on-click-modal="false" :close-on-press-escape="false" :visible.sync="dialogFormVisible" width='50%'>
    <div class="form-box">
      <el-form ref="reform" :model="reform":rules="rules" label-width="110px">
        <!-- <el-form-item label="认证账号：">
          <el-input v-model="reform.username"></el-input>
        </el-form-item> -->
        <el-form-item label="认证密码：" prop="privateStr" >
          <el-input v-model="reform.privateStr"></el-input>
        </el-form-item>
      </el-form>
    </div>
    <div slot="footer" class="dialog-footer">
      <el-button type="primary" @click="checkUserInfo('reform')">提 交</el-button>
...
export default {
  ...
  methods: {
    ...
    toLingqu(item){
      ...
      this.reform = {};
      this.lingquItem = item;
    },
    lingqu(userId) {
      let self = this
      let params = {
        wuLiaoId:this.lingquItem.id,
        status:2,
        getUserId:userId
      }
      this.postRequest("/stock/manager/get", params).then(resp => {
        if (resp && resp.status == 200 && resp.data.code == 0) {
          self.$message.success("领取成功");
          self.getCollectMouldList();
        }else {
          self.$message.error("领取失败");
        }
      })
    },
    checkUserInfo(formName) {
      let _this = this
      _this.checkLoading = true;
      if(_this.reform.privateStr+'' === 'undefined' || _this.reform.privateStr === ''){
        _this.$message.error("请填写验证密码")
        return;
      }
      let params = {
        userId:this.lingquItem.addUserId,
        privateStr:_this.reform.privateStr
      }
      this.postRequest('/stock/manager/check', params).then(resp => {
        _this.checkLoading = false
        if (resp && resp.status == 200) {
          _this.dialogFormVisible = false;
          this.lingqu(this.lingquItem.addUserId);
          this.getCollectMouldList();
        } else {
    ...
    getCollectMouldList() {
      let _this = this
      this.getRequest("/stock/manager/listbypage?page=" + this.currentPage + "&size=" + this.pagesize +"&status=1").then(resp => {
